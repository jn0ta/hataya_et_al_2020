...
def posterize(img: PILImage,
              v: float) -> PILImage:
    bits = int(8 - 8*v)                         # v(=mag) is in [0,1]
    augImg = ImageOps.posterize(img, bits)      # at this stage, 0 ~ 31 pixel values posterized to "0" (NOT 31)

    # adding suitable values to each pixels, make 0 ~ 31 pixel values posterized to "31"
    temp = np.uint8((int(2 ** (8 - bits)) - 1))
    w,h = augImg.size
    ones = np.ones((h,w,3),dtype=np.uint8)
    temp = Image.fromarray(ones * temp); del w,h,ones
    
    augImgNp = np.array(augImg)
    augImg = Image.fromarray(augImgNp + temp)
    return augImg
