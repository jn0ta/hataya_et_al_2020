...
def _rgb_to_hsv(img: torch.Tensor) -> torch.Tensor:
    return kornia.color.rgb_to_hsv(img) # kornia.rgb_to_hsv(img) # jun ota edition


def _hsv_to_rgb(img: torch.Tensor) -> torch.Tensor:
    return kornia.color.hsv_to_rgb(img) # kornia.hsv_to_rgb(img) # jun ota edition
...
def shear_x(img: torch.Tensor,
    ...
    return kornia.geometry.transform.shear(img, mag) # kornia.shear(img, mag) # jun ota edition
...
def translate_x(img: torch.Tensor,
    ...
    return kornia.geometry.transform.translate(img, mag) # kornia.translate(img, mag) # jun ota edition
...
def rotate(img: torch.Tensor,
           mag: torch.Tensor) -> torch.Tensor:
    return kornia.geometry.transform.rotate(img, mag) # kornia.rotate(img, mag) # jun ota edition
...
def posterize(img: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        bits = 8 - int(mag*8)
        mask = -int(2 ** (8 - bits))
        output = (img & mask) - mask - 1
    return ste((output.float()/255), mag)
