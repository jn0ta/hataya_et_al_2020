...
from .functional import (shear_x, shear_y, translate_x, translate_y, hflip, vflip, rotate, invert, solarize, posterize, # from functional import (shear_x, shear_y, translate_x, translate_y, hflip, vflip, rotate, invert, solarize, posterize, # jun ota edition
                         gray, contrast, auto_contrast, saturate, brightness, hue, sample_pairing, equalize, sharpness)
from .kernels import get_sharpness_kernel # from kernels import get_sharpness_kernel # jun ota edition
...
class _Operation(nn.Module):
    ...
    def __init__(self,
        ...
        #self._py_magnitude = initial_magnitude # jun ota edition
        # jun ota edition start
        if initial_magnitude is not None:
            self._py_magnitude = initial_magnitude * magnitude_scale # jun ota edition
        else:
...
class Posterize(_Operation):
    def __init__(self,
                 ...
                 #magnitude_scale : float = 4, # jun ota edition
...
class _KernelOperation(_Operation):
    ...
    # jun ota memo: probably, "self._original_operation is good, but self.operation w.r.t. setting a kernel to use"
    def _operation(self,
                   img: torch.Tensor,
